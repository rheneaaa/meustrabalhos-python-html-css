...
import mysql.connector # Importa mysql.connector para conectar o banco de dados ao python
from mysql.connector import Error

def Conectar():#Função faz a conecão com o Banco tem que estar em todas as outra Funções
    try: # Mostrar algum erro no código, nesse caso vai mostrar "Erro de Conexão"
        global con#É oq permite usar a Função Conectar dentro das outras Funcões
        con = mysql.connector.connect(host='localhost', database='testbra_park', user='root', password='') #Conexão com o banco
...
def Colsulta():#Função faz consulta ao Banco
    try:
        ...
        cursor = con.cursor()# Cursor é um objeto que permite fazer iteração para os elementos da tabela retornados e ler individualmente cada um dos objetos
        cursor.execute(consulta_sql) # Aqui ele executa o banco de dados atual
        linhas = cursor.fetchall() #Aqui ele faz o retorno da variável
